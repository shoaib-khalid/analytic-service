package com.kalsym.analytic.service.filter;

import com.kalsym.analytic.service.model.HttpReponse;
import com.kalsym.analytic.service.model.MySQLUserDetails;
import com.kalsym.analytic.service.model.Auth;
import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.kalsym.analytic.service.AnalyticServiceApplication;
import com.kalsym.analytic.service.service.MySQLUserDetailsService;
import com.kalsym.analytic.service.utils.Logger;
import com.kalsym.analytic.service.utils.DateTimeUtil;
...
public class SessionRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        Logger.application.info(Logger.pattern, AnalyticServiceApplication.VERSION, "------------- " + request.getMethod() + " " + logprefix + "-------------", "", "");

        final String authHeader = request.getHeader("Authorization");
        Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "Authorization: " + authHeader, "");
        ...
        if (null != authHeader && authHeader.startsWith("Bearer ")) {
            ...
            Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "token: " + accessToken, "");
            Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "token length: " + accessToken.length(), "");
            tokenPresent = true;
        } else {
            Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "token does not begin with Bearer String", "");
        }
            ...
            try {
                ...
                if (null != expiryTime && null != username) {
                    ...
                    try {
                        ...
                        Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "error calculating time to session expiry", "");
                    }
                    Logger.application.info(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "time to session expiry: " + diff + "ms", "");
                    if (0 < diff) {
                        authorized = true;
                        ...
                        Logger.application.warn(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "session expired", "");
                        //response.setStatus(HttpStatus.UNAUTHORIZED);
                        response.getWriter().append("Session expired");
                    }
                }
            } catch (IOException | IllegalArgumentException | RestClientException e) {
                Logger.application.error(Logger.pattern, AnalyticServiceApplication.VERSION, logprefix, "Exception processing session ", "", e);
